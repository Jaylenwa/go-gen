...
func genUtilsHttpClient(req GenReq) {

	str := "package utils\n\nimport (\n\t\"bytes\"\n\t\"context\"\n\t\"crypto/tls\"\n\t\"io\"\n\t\"net/http\"\n\t\"sync\"\n\t\"time\"\n)\n\ntype HttpClientInterface interface {\n\tGet(ctx context.Context, url string, headers map[string]string) (statusCode int, resBody []byte, err error)\n\tPut(ctx context.Context, url string, headers map[string]string, reqBody []byte) (statusCode int, resBody []byte, err error)\n\tPost(ctx context.Context, url string, headers map[string]string, reqBody []byte) (statusCode int, resBody []byte, err error)\n}\n\ntype HttpClient struct {\n\tclient *http.Client\n}\n\nvar (\n\thttpClientOnce sync.Once\n\tHttpClientImpl HttpClientInterface\n)\n\nfunc NewHttpClient() HttpClientInterface {\n\thttpClientOnce.Do(func() {\n\t\ttr := &http.Transport{\n\t\t\tTLSClientConfig: &tls.Config{InsecureSkipVerify: false},\n\t\t\tMaxIdleConns:    100,\n\t\t\tIdleConnTimeout: time.Minute,\n\t\t}\n\t\tHttpClientImpl = &HttpClient{\n\t\t\tclient: &http.Client{Transport: tr},\n\t\t}\n\t})\n\n\treturn HttpClientImpl\n}\n\nvar _ HttpClientInterface = &HttpClient{}\n\nfunc (httpClient *HttpClient) Get(ctx context.Context, url string, headers map[string]string) (statusCode int, respBody []byte, err error) {\n\tvar request *http.Request\n\tvar response *http.Response\n\n\trequest, err = http.NewRequest(\"GET\", url, nil)\n\tif err != nil {\n\t\treturn\n\t}\n\n\trequest = request.WithContext(ctx)\n\trequest.Header.Set(\"Content-Type\", \"application/json; charset=utf-8\")\n\tfor k, v := range headers {\n\t\trequest.Header.Set(k, v)\n\t}\n\n\tresponse, err = httpClient.client.Do(request)\n\tif err != nil {\n\t\treturn\n\t}\n\n\tdefer func() {\n\t\t_ = response.Close\n\t}()\n\n\trespBody, err = io.ReadAll(response.Body)\n\tif err != nil {\n\t\treturn\n\t}\n\n\tstatusCode = response.StatusCode\n\treturn\n}\n\nfunc (httpClient *HttpClient) Put(ctx context.Context, url string, headers map[string]string, reqBody []byte) (statusCode int, respBody []byte, err error) {\n\tvar request *http.Request\n\tvar response *http.Response\n\n\trequest, err = http.NewRequest(\"PUT\", url, bytes.NewBuffer(reqBody))\n\tif err != nil {\n\t\treturn\n\t}\n\n\trequest = request.WithContext(ctx)\n\trequest.Header.Set(\"Content-Type\", \"application/json; charset=utf-8\")\n\tfor k, v := range headers {\n\t\trequest.Header.Set(k, v)\n\t}\n\n\tresponse, err = httpClient.client.Do(request)\n\tif err != nil {\n\t\treturn\n\t}\n\n\tdefer func() {\n\t\t_ = response.Close\n\t}()\n\n\trespBody, err = io.ReadAll(response.Body)\n\tif err != nil {\n\t\treturn\n\t}\n\n\tstatusCode = response.StatusCode\n\treturn\n}\n\nfunc (httpClient *HttpClient) Post(ctx context.Context, url string, headers map[string]string, reqBody []byte) (statusCode int, respBody []byte, err error) {\n\tvar request *http.Request\n\tvar response *http.Response\n\n\trequest, err = http.NewRequest(\"PUT\", url, bytes.NewBuffer(reqBody))\n\tif err != nil {\n\t\treturn\n\t}\n\n\trequest = request.WithContext(ctx)\n\trequest.Header.Set(\"Content-Type\", \"application/json; charset=utf-8\")\n\tfor k, v := range headers {\n\t\trequest.Header.Set(k, v)\n\t}\n\n\tresponse, err = httpClient.client.Do(request)\n\tif err != nil {\n\t\treturn\n\t}\n\n\tdefer func() {\n\t\t_ = response.Close\n\t}()\n\n\trespBody, err = io.ReadAll(response.Body)\n\tif err != nil {\n\t\treturn\n\t}\n\n\tstatusCode = response.StatusCode\n\treturn\n}\n"
	path := req.HttpClientDir + "/request.go"
	if err := gfile.PutContents(path, strings.TrimSpace(str)); err != nil {
		mlog.Fatalf("writing content to '%s' failed: %v", path, err)
	} else {
		utils.GoFmt(path)
